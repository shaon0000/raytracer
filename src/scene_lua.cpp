...
int gr_nh_sphere_cmd(lua_State* L)
{
  ...
  data->node = new GeometryNode(name, new NonhierSphere(Point3D(0, 0, 0), radius));
...
int gr_cone_cmd(lua_State* L)
{
  ...
  const char* name = luaL_checkstring(L, 1);
  Ray ray;
  ray.p = Point3D(0.0, 0.0, 0.0);
  ray.d = Vector3D(0.0, 0.0, 1.0);

  data->node = new GeometryNode(name, new Cone(ray));
...
int gr_square_cmd(lua_State* L)
{
  ...
  const char* name = luaL_checkstring(L, 1);
  data->node = new GeometryNode(name, new Wall());
...
int gr_material_cmd(lua_State* L)
{
  ...
  double snell_N = luaL_checknumber(L, 4);
  double trans = luaL_checknumber(L, 5);

  data->material = new PhongMaterial(Colour(kd[0], kd[1], kd[2]),
                                     Colour(ks[0], ks[1], ks[2]),
                                     shininess, snell_N, trans);
...
// Associate a node with a light. When that node is being rendered, it will
// take on the colour of the light.
extern "C"
int gr_node_add_light_child_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
...
// Set a light's area
extern "C"
int gr_node_set_light_area_cmd(lua_State* L)
{
  GRLUA_DEBUG_CALL;
  ...
  Light* self = dynamic_cast<Light*>(selfdata->light);

  luaL_argcheck(L, self != 0, 1, "failed to convert it to Light instance");
...
static const luaL_reg grlib_functions[] = {
  ...
  {"associate", gr_node_add_light_child_cmd},
  {0, 0}
};
...
static const luaL_reg grlib_node_methods[] = {
  ...
  {"set_texture_scale", gr_node_set_texture_scale_cmd},
  {"scale", gr_node_scale_cmd},
  {"rotate", gr_node_rotate_cmd},
  {"translate", gr_node_translate_cmd},
  {"render", gr_render_cmd},
  {"set_area", gr_node_set_light_area_cmd},
  {0, 0}
};
